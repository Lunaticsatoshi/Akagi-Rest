...
export const parser = async (authHeader: string, mimicId: string): Promise<admin.auth.DecodedIdToken> => {
...
export const parseToken: any = async (req: FastifyRequest, _res: FastifyReply, next: () => void) => {
  ...
  try {
    const decodedToken: admin.auth.DecodedIdToken = await parser(Authorization, mimicId)
